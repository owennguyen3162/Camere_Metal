...
class VideoWriter {
    ...
    lazy var presetSettingEncoder: AVOutputSettingsAssistant? = {
        return VideoEncoderEncoderSettings.Preset1920x1080.configuration()
    }()
...
//    }
    ...
    func stopWriting(isEnabledWriteVideo: Bool = true, blockCompletion: @escaping (_ url: URL) -> ()) {
        self.videoInputWriter?.markAsFinished()
        self.audioInputWriter?.markAsFinished()
        if(isEnabledWriteVideo){
